...
// Hapus SEMUA baris yang import dari widgets
const lines = c.split('\n');
const filtered = lines.filter(line => !line.includes("from './widgets"));
c = filtered.join('\n');

// Cari 'export default function App'
const appIdx = c.indexOf('\nexport default function App');

// Cek apakah sudah ada inline functions
if (c.includes('function TaskTracker')) {
  // Ada duplikasi di dalam App function — restore clean
  console.log('Already has TaskTracker — restoring clean first');
...
// Insert inline functions before App()
const allInlines = inlineTask + inlineDraft + inlineEnc;
c = c.slice(0, appIdx) + '\n' + allInlines + '\n' + c.slice(appIdx);
