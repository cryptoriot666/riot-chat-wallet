        ...
        if isinstance(msg, dict) and msg.get("role") == "user":
            ...
            for pattern, label in patterns:
                ...
                if m and is_valid_name(m.group(1)):
                    print(f"[EXTRACT] Γ£ô '{label}': {m.group(1)}")
                    return m.group(1)
            ...
            if len(words) == 1:
                w = words[0]
                if is_valid_name(w):
                    print(f"[EXTRACT] Γ£ô single word: {w}")
                    return w

    print("[EXTRACT] Γ£ù No valid name found")
    return ""
...
def update_profile_name(wallet_hash, name):
    if not name or not wallet_hash or not name.strip():
        print(f"[UPDATE_NAME] Γ£ù Skipped: empty name")
        return False

    name = name.strip()
    if not is_valid_name(name):
        print(f"[UPDATE_NAME] Γ£ù Rejected invalid name: '{name}'")
        return False
    ...
    if USE_SQLITE:
        ...
        row = c.fetchone()
        if row:
            print(f"[UPDATE_NAME] Old name: '{row[0] or ''}' ΓåÆ New: '{name}'")
    ...
    conn.commit()
    conn.close()
    print(f"[UPDATE_NAME] Γ£ô Success: '{name}' saved")
    return True
...
def update_profile_settings(wallet_hash, settings):
    ...
    conn.commit()
    conn.close()
    print(f"[UPDATE_SETTINGS] Γ£ô Updated {list(updates.keys())} for {wallet_hash}")
    return True
            ...
            try:
                ...
                if res.status_code in [200, 202]:
                    ...
                    if "newlyCreated" in result:
                        ...
                        blob_network = label
                        print(f"[SAVE_MEMORY] Γ£ô {label}: {blob_id[:20]}...")
                        break
                    elif "alreadyCertified" in result:
                        blob_id = result["alreadyCertified"]["blobId"]
                        blob_network = label
                        print(f"[SAVE_MEMORY] Γ£ô {label} existing: {blob_id[:20]}...")
                        break
                else:
            ...
            if current_agent and previous_agent and current_agent != previous_agent:
                ...
                if cross_agent_bus:
                    ...
                    print(f"[SAVE_MEMORY] Cross-agent handoff recorded: "
                          f"{previous_agent} ΓåÆ {current_agent}")
        except Exception as e:
...
def walrus_store_direct():
    ...
    try:
            ...
            try:
                ...
                if res.status_code in [200, 202]:
                    ...
                    if "newlyCreated" in result:
                        ...
                        print(f"[WALRUS_DIRECT] Γ£ô {label}: {blob_id[:20]}...")
                        break
                    elif "alreadyCertified" in result:
                        blob_id = result["alreadyCertified"]["blobId"]
                        is_new = False
                        actual_network = label
                        actual_aggregator = WALRUS_AGGREGATOR if label == "mainnet" else "https://aggregator.walrus-testnet.walrus.space"
                        print(f"[WALRUS_DIRECT] Γ£ô {label} existing: {blob_id[:20]}...")
                        break
                else:
        ...
        return jsonify({
            ...
            "cost_mist": cost_mist,
            "is_new": is_new,
            "url": f"{actual_aggregator}/v1/blobs/{blob_id}",  # ΓåÉ correct URL
            "network": actual_network,  # ΓåÉ "mainnet" atau "testnet"
            "raw": result
        })
